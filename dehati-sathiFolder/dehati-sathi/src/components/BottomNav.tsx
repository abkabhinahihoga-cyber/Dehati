    ...
    return (
        <div className="fixed bottom-0 left-0 w-full z-50 md:hidden pb-safe glass shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] border-t border-white/20">
            <div className="flex justify-around items-center h-16">
                ...
                <Link href="/reels" className={`flex flex-col items-center gap-1 w-1/5 ${isActive('/reels') ? 'text-pink-600' : 'text-gray-500'}`}>
                    <PlayCircle size={22} strokeWidth={isActive('/reels') ? 2.5 : 2} className={isActive('/reels') ? "fill-pink-50" : ""} />
                    <span className="text-[10px] font-medium">{t('reels')}</span>
                </Link>
